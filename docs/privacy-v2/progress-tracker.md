...
| 17 QA iOS build 25 failure investigation | `codex/qr-mob-017-ios-qa-build-25-failure` | — | `../worktrees/quiet-room-mobile-task-17-ios-qa-build-25-failure` | Codex | ready | Investigate why iOS QA TestFlight build `25` fails and determine whether the issue is build artifact/config, TestFlight install behavior, QA runtime config, Firebase/Auth, or a regression introduced after known-good build `23` | — | Start from `develop`, compare known-good QA build `23` against failing builds `24`/`25`, capture device/TestFlight symptoms and logs, verify QA env/Firebase/API/streaming config, inspect build/version metadata and signing, run release-simulator smoke tests, and document the exact root cause plus rollback/redeploy recommendation. Success criteria: a written diagnosis, reproduction steps, relevant logs/screenshots, commands run, and either a fix PR or a clear recommendation to remove bad builds and republish from the last known-good source. |
