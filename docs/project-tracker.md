...
| QR-MOB-018 | In Progress | High | Profile system / model evals | Evaluate split-profile generation quality with `gpt-5.5` and Anthropic Sonnet 4.6. | Backend worktree started at `/Users/mjreinig/projects/Gabriel_App/worktrees/Gabriel-qr-mob-018-split-profile-model-eval` on branch `codex/qr-mob-018-split-profile-model-eval`; source-of-truth docs are under `docs/qr-mob-018-split-profile-model-eval/`. Keep this separate from QR-MOB-009. Use the split-profile replay/eval harness to compare the current profile model against `gpt-5.5` and Sonnet 4.6, including any provider-broker/harness work needed for Anthropic profile-builder routes. Focus on generated profile quality, schema adherence, third-person voice, concision, temporal accuracy, emotional over-certainty, Catholic/spiritual framing, latency, and whether either model is worth a QA-only profile-builder rollout. |
...
| QR-MOB-020 | Done | High | iOS QA / TestFlight recovery | Investigate and recover from iOS QA TestFlight build `25` failure. | Completed on 2026-06-03. Root cause was stale ignored generated iOS native artifacts: a release-simulator build from the stale `ios/` tree blank-screened with `No component found for view with name "RNCSafeAreaProvider"`, while `npm run native:sync:qa -- ios` restored the `react-native-safe-area-context` Pod/codegen and the rebuilt app reached the Quiet Room home screen. Recovery build `26` passed QA config, TestFlight preflight, typecheck, release-simulator build/launch, device archive, entitlement verification, and local export. After restarting Xcode restored App Store Connect account access, the existing `build/ios-qa-b26.xcarchive` uploaded to App Store Connect/TestFlight; upload output reported `Uploaded package is processing`, `Upload succeeded`, `Uploaded QuietRoomQA`, and `** EXPORT SUCCEEDED **`. Source docs: `docs/qr-mob-020-ios-qa-build-25-failure/`. |
...
**2026-06-01 progress note:** Started in the Gabriel backend worktree `/Users/mjreinig/projects/Gabriel_App/worktrees/Gabriel-qr-mob-018-split-profile-model-eval` on branch `codex/qr-mob-018-split-profile-model-eval`. The QR-MOB-008 profile eval harness has been brought onto the current `develop-from-main` line, Anthropic non-streaming completion support has been added for eval use, and initial one-case current / `gpt-5.5` / Sonnet profile eval smokes completed. A 3-case Sonnet profile sample also completed and red-gated (`weighted_average=4.4`, `critical_fail_case_count=3`) due to second-person voice failures and one continuity reset. True split-profile `core` / `recent` three-way replays completed for one conversation, five steady-state conversations, one fresh-start bootstrap conversation, one fresh-start sparse conversation allowing empty `core`, one five-conversation no-bootstrap fresh-start replay, and one hesitant-growth replay with two short conversations followed by a larger one (`qr-mob-018-b71c-split-three-way-1conv`, `qr-mob-018-b71c-split-three-way-steady-5conv`, `qr-mob-018-b71c-split-three-way-fresh-1conv`, `qr-mob-018-b71c-split-three-way-fresh-empty-core-1conv`, `qr-mob-018-b71c-split-three-way-fresh-empty-core-5conv`, `qr-mob-018-b71c-split-three-way-hesitant-growth-3conv`), with direct-read output in ignored backend eval artifacts and privacy-safe retained summaries in the QR-MOB-018 docs. Early recommendation: do not change the default profile model yet; treat empty `core` as valid for sparse first-conversation evidence when `recent` is useful, but note that all three models can still create `core` from a short four-message conversation when the content sounds durable. Keep evaluating candidate-model verbosity/voice risks before any QA-only rollout. The retained work docs are under `docs/qr-mob-018-split-profile-model-eval/` in that backend worktree.
