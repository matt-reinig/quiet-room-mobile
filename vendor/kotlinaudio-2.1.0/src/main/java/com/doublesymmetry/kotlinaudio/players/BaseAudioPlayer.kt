...
import com.doublesymmetry.kotlinaudio.diagnostics.NativeHttpCaptureDataSourceFactory
import com.doublesymmetry.kotlinaudio.diagnostics.prepareNativeHttpCaptureHeaders
    ...
    protected fun getMediaSourceFromAudioItem(audioItem: AudioItem): MediaSource {
        ...
        val factory: DataSource.Factory = when {
            ...
            isUriLocalFile(uri) -> {
                ...
                val preparedHeaders = prepareNativeHttpCaptureHeaders(audioItem.options?.headers)
                val tempFactory = DefaultHttpDataSource.Factory().apply {
                    setUserAgent(userAgent)
                    setAllowCrossProtocolRedirects(true)

                    if (preparedHeaders.networkHeaders.isNotEmpty()) {
                        setDefaultRequestProperties(preparedHeaders.networkHeaders)
                    }
                }

                val captureContext = preparedHeaders.captureContext
                val httpFactory: DataSource.Factory = if (captureContext == null) {
                    tempFactory
                } else {
                    NativeHttpCaptureDataSourceFactory(context, tempFactory, captureContext)
                }
